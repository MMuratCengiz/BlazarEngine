SET(BULLET_BINARY_DIR ${PROJECT_BINARY_DIR}/bullet3/)
IF (${BLAZAR_INSTALL_LIBS})
    IF("${BLAZAR_INSTALL_LOCATION}" STREQUAL "")
        SET(BULLET_BINARY_DIR ${CMAKE_INSTALL_PREFIX}/)
    ELSE()
        SET(BULLET_BINARY_DIR ${BLAZAR_INSTALL_LOCATION})
    ENDIF()
ENDIF()

SET(BULLET_OPTIONS
        -DINSTALL_LIBS=on
        -DPKGCONFIG_INSTALL_PREFIX=${BULLET_BINARY_DIR}lib
        -DBUILD_BULLET2_DEMOS=OFF
        -DBUILD_CPU_DEMOS=OFF
        -DBUILD_EXTRAS=OFF
        -DBUILD_OPENGL3_DEMOS=OFF
        -DBUILD_UNIT_TESTS=OFF
        -DUSE_DOUBLE_PRECISION:BOOL=OFF
        -DCMAKE_INSTALL_PREFIX:PATH=${CMAKE_INSTALL_PREFIX}
		-DCMAKE_C_COMPILER=${CMAKE_C_COMPILER}
		-DCMAKE_CXX_COMPILER=${CMAKE_CXX_COMPILER}
        -DINCLUDE_INSTALL_DIR=${BULLET_BINARY_DIR}include/
        )

ExternalProject_Add(bullet3
        SOURCE_DIR "${PROJECT_SOURCE_DIR}/external/bullet3"
        CMAKE_ARGS ${BULLET_OPTIONS}
        BUILD_ALWAYS 1
        BUILD_IN_SOURCE 1
        CMAKE_CACHE_ARGS "-DCMAKE_C_COMPILER:FILEPATH=${CMAKE_C_COMPILER}" "-DCMAKE_CXX_COMPILER:FILEPATH=${CMAKE_CXX_COMPILER}"
        PREFIX "bullet3"
        CMAKE_GENERATOR ${CMAKE_GENERATOR}
        )

INCLUDE_DIRECTORIES("${BULLET_BINARY_DIR}include")
LINK_DIRECTORIES("${BULLET_BINARY_DIR}lib")

LIST(APPEND BlazarSources "${BULLET_BINARY_DIR}include")