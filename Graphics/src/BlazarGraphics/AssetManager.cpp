...
{
    auto litCube = std::make_unique< LitCubePrimitive >( );
    auto plainCube = std::make_unique< PlainCubePrimitive >( );
    auto plainSquare = std::make_unique< PlainSquarePrimitive >( );
    auto texturedSquare = std::make_unique< TexturedSquarePrimitive >( );
    auto plainTriangle = std::make_unique< PlainTrianglePrimitive >( );

    builtinPrimitivePathMap[ litCube->getPath( ) ] = std::move( litCube );
    builtinPrimitivePathMap[ plainCube->getPath( ) ] = std::move( plainCube );
    builtinPrimitivePathMap[ plainSquare->getPath( ) ] = std::move( plainSquare );
    builtinPrimitivePathMap[ texturedSquare->getPath( ) ] = std::move( texturedSquare );
    builtinPrimitivePathMap[ plainTriangle->getPath( ) ] = std::move( plainTriangle );

    geometryTable.resize( builtinPrimitivePathMap.size( ) );

    for ( auto &primitive: builtinPrimitivePathMap )
    {
        const std::string &primitivePath = primitive.second->getPath( );
        auto primitiveIdx = ( uint32_t ) primitive.second->getType( );
        geometryTable[ primitiveIdx ] = { };
        MeshGeometry &primitiveGeometry = geometryTable[ primitiveIdx ];
        SubMeshGeometry &primitiveSubMesh = primitiveGeometry.subGeometries.emplace_back( );
        primitiveSubMesh.dataRaw = primitive.second->getData( );
        primitiveSubMesh.vertexCount = primitive.second->getVertexCount( );
    }
}
...
{
    auto builtinPrimitive = builtinPrimitivePathMap.find( meshPath );
    if ( builtinPrimitive != builtinPrimitivePathMap.end( ) )
    {
        attachToEntity->createComponent< ECS::CMaterial >( );

        auto mesh = attachToEntity->createComponent< ECS::CMesh >( );
        mesh->path = meshPath;
        mesh->geometryRefIdx = ( int ) builtinPrimitive->second->getType( );
    }
    else
    {
...
void AssetManager::packSubGeometry( SubMeshGeometry &geometry )
{
    ...
    for ( int i = 0, texIdx = 0, boneIdx = 0; i < geometry.vertexCount * 3; i += 3, texIdx += 2, boneIdx += 4 )
    {
        ...
        geometry.dataRaw.push_back( 1.0f );
        pushToDataRaw( geometry.normals, i, 3 );
        geometry.dataRaw.push_back( 0.0f );
...
{
    return geometryTable[ ( uint32_t ) primitive ];
}
