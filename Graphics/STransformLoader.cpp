...
STransformLoader::STransformLoader( std::shared_ptr< InstanceContext > context ) : context( std::move( context ) ) {
...
void STransformLoader::load( const std::shared_ptr< ECS::CTransform > &transform ) {
    glm::mat4 modelMatrix { 1 };

    glm::translate( modelMatrix, transform->position );
    glm::scale( modelMatrix, transform->scale );
