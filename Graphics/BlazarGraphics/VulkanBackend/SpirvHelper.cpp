...
std::vector< uint32_t > SpirvHelper::GLSLtoSPV( const vk::ShaderStageFlagBits shaderType, const char *pShader )
{
    ...
    {
        Core::Logger::get().log( Core::Verbosity::Warning, "Graphics", std::string( shader.getInfoLog( ) ) );
        Core::Logger::get().log( Core::Verbosity::Warning, "Graphics", std::string( shader.getInfoDebugLog( ) ) );
        return spirv;
    }
